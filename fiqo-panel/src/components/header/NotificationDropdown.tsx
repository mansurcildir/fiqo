...
export default function NotificationDropdown({ notifying, notifications, avatarSrc, fetchNotifications }: Props) {
  ...
  const readNotification = (userUuid: string) => {
    notificationAPI.readNotification(userUuid).then(() => {
      fetchNotifications();
    });
  };

  const deleteNotification = (notificationUuid: string) => {
    notificationAPI.deleteNotification(notificationUuid).then(() => {
      fetchNotifications();
    });
  };
        ...
        <ul className="custom-scrollbar flex h-auto flex-col overflow-y-auto">
          {notifications?.map((notification) => (
            <li key={notification.uuid}>
              <DropdownItem
                onClick={() => readNotification(notification.uuid)}
              ...
              >
                <span className="relative z-1 block h-10 w-full max-w-10 rounded-full">
                  ...
                  />
                  <span
                    className={`${notification.read ? 'bg-success-500' : 'bg-error-500'} absolute right-0 bottom-0 z-10 h-2.5 w-full max-w-2.5 rounded-full border-[1.5px] border-white dark:border-gray-900`}
                  ></span>
                </span>
